...
（无——0824-only 全量对账已于 2026-09-09 收官）

| 20260824 版任务 | 成果 |
|------|------|
| 哔哩哔哩视频（含周杰伦两合集/红警3两套OST/怒之铁拳4/AI续写系列） | 2951 |
| 音乐卡 16G 全量（排除 vgm/midi音乐/备份） | 1102 |
| 音乐档案游戏音乐（赛尔号/PopKart/QQ系/梦4/铸剑物語）+ 召唤之夜 + 喜马拉雅 + qq网易云 | 4559 |
| AI音乐合集 0824版 / 0726版（双版本对照） | 498 / 146 |
| x88 放大300 双版本对照 | 262 |
| 版本对比实验 | 3 |
| 网络视频-2026（DNF/愤怒小鸟/卓越之剑/CSOL/彩虹岛/植物大战僵尸等） | 697 |
| 酷我全量 0824 重转（对账 1536 + 7月版专辑全量补转 3205） | 4741 |

### 2026-09-09 0824-only 全量对账完成

- **口径修正**：旧版本转换的不算，只有 0824 的才算——去重索引只扫 `20260824\*.mid`，7 月版转过的全部重转
- run_two_worker.py 双 worker（GPU0 扫 AI、GPU1 排队网络视频+酷我），累计补转 **约 3300 首**（网络视频收尾 + 酷我 7 月版专辑全量）
- **20260824 目录最终 14533 首**（约 260MB），全库累计约 31000 首
- 失败仅 10 首：超长视频 tick 溢出 3（愤怒小鸟进化/卓越之剑 01-30、31-60）、CSOL 特殊编码 2、DNF 树洞/Digilio Dreamer/冒险岛神木村 特殊编码 mp3 3（可 ffmpeg 预转补跑）、卓期溢出遗留 2

### 2026-09 工程修复记录（run_two_worker 期间）

1. **超长路径复制失败**（655 首批量失败）：酷我深层专辑路径超 260，`_winapi.CopyFile2` WinError 3。修复：worker 源路径超 200 字符时先 `\\?\` 前缀复制成短名（_bili_newver_worker.py）
2. **rmtree 静默泄漏**：转完超长曲目后 `shutil.rmtree(ignore_errors=True)` 对深层 stem 路径删不掉，累计泄漏 12~16GB/次。修复：`\\?\` 前缀强制删除（run_two_worker.py）
3. **会话后台进程被杀**：run_in_background 启动的 worker 随回合结束被连带杀死。修复：powershell Start-Process 完全脱离会话启动
4. **日志锁冲突**：cmd 重定向句柄与脚本自身写日志冲突 → PermissionError。脚本自己写日志，不要外加重定向
5. **F/G 源盘掉线**：外置盘离线时 worker 扫描出"待转 0"直接退出。恢复挂载后重启即续跑

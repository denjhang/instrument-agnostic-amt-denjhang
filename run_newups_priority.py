#!/usr/bin/env python3
"""逆转逆转再逆转(13) + 紫翼基拉(40) 并行优先转换 单worker GPU0"""
...
SRC2 = Path(r"G:\网络视频-2026\哔哩哔哩视频\紫翼基拉")
...
files += sorted(f for f in SRC2.rglob("*") if f.is_file() and f.suffix.lower() in
               {".mp3",".wav",".flac",".ogg",".m4a",".aac",".wma",".aiff",".aif",".opus",".mp4",".mkv",".flv",".webm",".mov",".avi",".ts",".m4v"})
pending = [f for f in files if not (expected_final(f).exists() and expected_final(f).stat().st_size > 100)]
print(f"逆转逆转再逆转: 总{len(files)} 待转{len(pending)}", flush=True)
...
print(f"新UP主完成: {done}/{len(pending)} 失败{failed}", flush=True)

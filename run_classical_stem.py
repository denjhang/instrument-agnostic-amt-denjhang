...
SOURCE_BASE = Path(r"E:\存储器备份\2024.10.30 音乐卡16G")
SOURCE_DIRS = ["赋格的艺术","网络收集","网络收集2","无伴奏曲组","音乐的奉献"]
TARGET_BASE = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260729")
WORK_DIR = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260729\_work_classical")

AUDIO_EXTS = {".mp3",".wav",".flac",".ogg",".m4a",".aac",".wma",".aiff",".aif",".opus"}
LOG_FILE = ROOT / "run_classical_stem.log"
GPUS = [0,1]
WORKER_SCRIPT = NEW_DIR / "_classical_stem_worker.py"
...
def find_all_audios():
    ...
    for dn in SOURCE_DIRS:
        d = SOURCE_BASE / dn
        if not d.exists():
            log(f"⚠ 不存在: {dn}"); continue
        for f in sorted(d.rglob("*")):
            if f.is_file() and f.suffix.lower() in AUDIO_EXTS:
                result.append((f, dn))
    return result

def expected_final_midi(audio, dn):
    """保留完整源目录结构: TARGET_BASE/<目录名>/<相对子路径>.mid"""
    rel = audio.relative_to(SOURCE_BASE / dn)
    final = TARGET_BASE / dn / rel.with_suffix(".mid")
...
def worker_proc(task_queue, result_queue, gpu_id):
    ...
    while True:
        ...
        idx, total, audio, dn, final = item
        song_stem = audio.stem
        disp = f"{dn}/{audio.name}"
        work_name = "s" + hashlib.md5(disp.encode("utf-8")).hexdigest()[:12]
        ...
        try:
            ...
            short_stem = "s" + hashlib.md5(song_stem.encode("utf-8")).hexdigest()[:10]
...
def main():
    ...
    log("="*50); log("古典音乐多目录 - 分轨+力度转换"); log("="*50)
    log(f"源: {SOURCE_BASE}"); log(f"目标: {TARGET_BASE}")
    write_worker_script()
    all_audios = find_all_audios()
    log(f"音频总数: {len(all_audios)}")
    from collections import Counter
    for dn, c in sorted(Counter(dn for _,dn in all_audios).items()):
        log(f"  ({c}) {dn}")
    tasks = [(a, dn, expected_final_midi(a, dn)) for a, dn in all_audios if not (expected_final_midi(a, dn).exists() and expected_final_midi(a, dn).stat().st_size>100)]
    log(f"待转换: {len(tasks)}")
    if args.dry_run:
        for a,dn,final in tasks[:5]: log(f"  {dn}/{a.name} -> {final.relative_to(TARGET_BASE)}")
        if len(tasks)>5: log(f"  ... 共 {len(tasks)}")
        return
    if not tasks: log("全部完成!"); return
    from multiprocessing import Process, Queue
    total = len(tasks); tq, rq = Queue(), Queue()
    for i,(a,dn,final) in enumerate(tasks,1): tq.put((i,total,a,dn,final))

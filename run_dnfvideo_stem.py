...
SOURCE = Path(r"G:\网络视频-2026\DNF背景音乐(旧版)合集--部分音乐加了npc语音和互动音效")
TARGET = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260727\DNF背景音乐(旧版)合集")
WORK_DIR = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260727\_work_dnfvideo")
VIDEO_EXTS = {".mp4",".mkv",".avi",".mov",".flv",".webm"}
AUDIO_EXTS = {".mp3",".wav",".flac",".ogg",".m4a",".aac",".wma",".aiff",".aif",".opus"}
ALL_EXTS = VIDEO_EXTS | AUDIO_EXTS
LOG_FILE = ROOT / "run_dnfvideo_stem.log"
GPUS = [0,1]
WORKER_SCRIPT = NEW_DIR / "_dnfvideo_stem_worker.py"
...
short_stem = "d" + hashlib.md5(orig_stem.encode("utf-8")).hexdigest()[:10]
...
def worker_proc(task_queue, result_queue, gpu_id):
    ...
    while True:
        ...
        work_name = "d" + hashlib.md5(disp.encode("utf-8")).hexdigest()[:12]
        ...
        try:
            ...
            short_stem = "d" + hashlib.md5(song_stem.encode("utf-8")).hexdigest()[:10]
...
def main():
    ...
    log("="*50); log("DNF背景音乐(旧版)合集 - 分轨+力度"); log(f"源: {SOURCE}"); log(f"目标: {TARGET}")
    write_worker_script()

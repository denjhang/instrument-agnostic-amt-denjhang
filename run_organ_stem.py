...
输出: 20260729\管风琴/<专辑>/<完整源子路径>/<曲名>.mid  （独立日期目录, 不污染原 pianotrans MIDI）
"""
...
SOURCE = Path(r"E:\存储器备份\2024.10.30 音乐卡16G\管风琴")
TARGET = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260729\管风琴")
WORK_DIR = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260729\管风琴\_work_organ")

AUDIO_EXTS = {".mp3",".wav",".flac",".ogg",".m4a",".aac",".wma",".aiff",".aif",".opus"}
LOG_FILE = ROOT / "run_organ_stem.log"
GPUS = [0,1]
WORKER_SCRIPT = NEW_DIR / "_organ_stem_worker.py"
...
short_stem = "o" + hashlib.md5(orig_stem.encode("utf-8")).hexdigest()[:10]
...
def worker_proc(task_queue, result_queue, gpu_id):
    ...
    while True:
        ...
        work_name = "o" + hashlib.md5(disp.encode("utf-8")).hexdigest()[:12]
        ...
        try:
            ...
            short_stem = "o" + hashlib.md5(song_stem.encode("utf-8")).hexdigest()[:10]

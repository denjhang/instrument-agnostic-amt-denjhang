#!/usr/bin/env python3
#!/usr/bin/env python3
r"""新版(20260823)音乐卡批量转换 F:\KwDownload\song（视频+音频混合）
...
WORKER_SCRIPT = ROOT / "_kuwo_newver_worker.py"
LOG_FILE = ROOT / "run_kuwo_newver.log"

SOURCE = Path(r"F:\KwDownload\song")
TARGET = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260824\酷我")
WORK_DIR = TARGET / "_work8"
...
OLD_TARGET = Path(r"D:/开源合集/开源 2022.9.14/芯片音乐/自制芯片音乐/MIDI识别合集/MIDI识别 2026/20260726")
...
def find_files():
    ...
    for f in sorted(SOURCE.rglob("*")):
        ...
        if any(p in EXCLUDE for p in rel.parts):
            continue
        # 7月版已转过整个专辑的跳过(产物在20260726下)
        if (OLD_TARGET / rel.parts[0]).exists():
            continue
        files.append(f)
    return files

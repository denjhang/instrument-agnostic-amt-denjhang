#!/usr/bin/env python3
"""彩虹岛LaTale - 分轨+力度转换
源: G:\\网络视频-2026\\彩虹岛LaTale--部分音乐加了npc语音和互动音效 (190个mp4, 平铺)
输出: 20260727/彩虹岛LaTale/<曲名>.mid  (按游戏音乐输出)
...
SOURCE = Path(r"G:\网络视频-2026\【LaTale】（彩虹岛）主要  “BGM较完整合集”被游戏耽误的音乐公司")
TARGET = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260727\彩虹岛LaTale")
WORK_DIR = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260727\_work_latale")
VIDEO_EXTS = {".mp4",".mkv",".avi",".mov",".flv",".webm"}
AUDIO_EXTS = {".mp3",".wav",".flac",".ogg",".m4a",".aac",".wma",".aiff",".aif",".opus"}
ALL_EXTS = VIDEO_EXTS | AUDIO_EXTS
LOG_FILE = ROOT / "run_latale_stem.log"
GPUS = [0,1]
WORKER_SCRIPT = NEW_DIR / "_latale_stem_worker.py"
...
def worker_proc(task_queue, result_queue, gpu_id):
    ...
    while True:
        ...
        work_name = "l" + hashlib.md5(disp.encode("utf-8")).hexdigest()[:12]
        ...
        try:
            ...
            short_stem = "l" + hashlib.md5(song_stem.encode("utf-8")).hexdigest()[:10]
...
def main():
    ...
    log("="*50); log("彩虹岛LaTale - 分轨+力度"); log(f"源: {SOURCE}"); log(f"目标: {TARGET}")
    write_worker_script()

#!/usr/bin/env python3
"""冒险岛BGM - 分轨+力度转换
源: G:\\网络视频-2026\\冒险岛BGM--部分音乐加了npc语音和互动音效 (190个mp4, 平铺)
输出: 20260727/冒险岛BGM/<曲名>.mid  (按游戏音乐输出)
...
SOURCE = Path(r"G:\网络视频-2026\【冒险岛BGM】超详细整理(按区域) [持续更新...]")
TARGET = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260727\冒险岛BGM(按区域)")
WORK_DIR = Path(r"D:\开源合集\开源 2022.9.14\芯片音乐\自制芯片音乐\MIDI识别合集\MIDI识别 2026\20260727\_work_maplestory")
VIDEO_EXTS = {".mp4",".mkv",".avi",".mov",".flv",".webm"}
AUDIO_EXTS = {".mp3",".wav",".flac",".ogg",".m4a",".aac",".wma",".aiff",".aif",".opus"}
ALL_EXTS = VIDEO_EXTS | AUDIO_EXTS
LOG_FILE = ROOT / "run_maplestory_stem.log"
GPUS = [0,1]
WORKER_SCRIPT = NEW_DIR / "_maplestory_stem_worker.py"
...
short_stem = "m" + hashlib.md5(orig_stem.encode("utf-8")).hexdigest()[:10]
...
def worker_proc(task_queue, result_queue, gpu_id):
    ...
    while True:
        ...
        work_name = "m" + hashlib.md5(disp.encode("utf-8")).hexdigest()[:12]
        ...
        try:
            ...
            short_stem = "m" + hashlib.md5(song_stem.encode("utf-8")).hexdigest()[:10]
...
def main():
    ...
    log("="*50); log("冒险岛BGM - 分轨+力度"); log(f"源: {SOURCE}"); log(f"目标: {TARGET}")
    write_worker_script()
